...
class ProductProvider extends ChangeNotifier {
  ...
  List<PurchaseModel> purchaseListOfSpecifiProduct = [];
  ...
  getAllPurchaseByProductId(String pid) {
    DbHelper.getAllPurchaseByProducts(pid).listen((snapshot) {
      purchaseListOfSpecifiProduct = List.generate(snapshot.docs.length,
          (index) => PurchaseModel.fromMap(snapshot.docs[index].data()));
      notifyListeners();
    });
  }

  Stream<DocumentSnapshot<Map<String, dynamic>>> getProductsById(String id) =>
      DbHelper.getProductsById(id);
